...
class BaseBot(ABC):
    ...
    def report(self, text):
        ...
        if self.print_all:
            self.real_print(text)
        elif text.startswith("Total score is"):
            score = re.sub("\D", "", text)
    ...
    def _mock_input(self, *args, **kwargs):
        ...
        elif self.last_print == "Enter dice to keep (no spaces), or (q)uit: ":
    ...
    def _enter_dice(self):
       ...
       # roll = str(self.last_roll)


        roll_string = ""
        # change the roll variable in the for to self.last_roll
...
class YourBot(BaseBot):
    ...
    def _enter_dice(self):
        """simulate user entering which dice to keep.
        Defaults to all scoring dice"""
        x= sorted(self.last_roll)
        x = Counter(self.last_roll).most_common()
